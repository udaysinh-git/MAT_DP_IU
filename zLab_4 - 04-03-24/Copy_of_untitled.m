...
frequencies = linspace(-Fs/2, Fs/2 - df, N);
MT_fft = abs(Am) * sinc(frequencies * 0.05 / (2*pi)); % Fourier Transform of mt

% Compute Fourier Transform of the Modulated Signal
MS_fft = abs(Ac) * abs(Am) * (sinc((frequencies - 0.5) / (2*pi)) + sinc((frequencies + 0.5) / (2*pi))); % Fourier Transform of MS
